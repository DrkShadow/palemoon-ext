...
const searchGroups = Object.freeze([
	...
	#cookieNoticeModal.vrm-reveal[style*="block"] .vrm-reveal__icon--close'.replace("\n\t", ""),
	...
	#sanoma-consent-accept-button'.replace("\n\t", ""),
	...
	.c-cookie-consent .c-cookie-consent__button'.replace("\n\t", ""),
...
const searchPairs = Object.freeze((function() {
		const spairs = {};
		for (const selector in searchPairLists) {
			spairs[selector] = searchPairLists[selector].join(',');
		}
		return spairs;
	})());
...
function searchLoop(timeoutDuration, tries) {
	...
	for (const box of document.querySelectorAll(searchPairsJoinedKeys)) {
		for (const selector of searchPairKeys) {
			...
			const buttons = (box.shadowRoot || box).querySelectorAll(searchPairs[selector]);
